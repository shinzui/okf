...
- The built-in profile registry now pins `mori://shinzui/okf-profiles` v0.14.0
  (from v0.10.0), adding `assurance.reviews`, `assurance.failureModes`, and
  `documentation.userDocumentation` to the default catalogue.
...
- Profile commands can read several registries at once. Repeat `--registry`, set
  ...
  provenance in text and JSON. A failed source no longer hides profiles from
  sources that loaded, while named lookups fail closed and reject duplicate
  exports rather than guessing.
- `okf bundles` lists discovered bundle paths without opening `fzf`; `--json`
...
- The legacy `OKF_PROFILE_REGISTRY` variable and `profiles.registry`
  configuration shape remain supported as one-element source lists. Exact
...
- Commands that consume an existing bundle now accept an omitted `BUNDLE` and
  ...
  both `id` forms. Explicit paths bypass picker detection and process spawning,
  preserving non-interactive scripts. `id next PREFIX` is the new interactive
  one-positional form; `id next BUNDLE PREFIX` keeps its existing meaning.
