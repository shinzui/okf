...
### Every generated page records its producer

Each page carries the OKF v0.2 `generated` family naming the process that wrote
it:
...
so straight out of the generator, with no extra flag, strict validation passes:
...
exit code `0`. The default actor is deliberately version-free: `okf/0.5.0.0`
would also be a legal actor, but it would change every generated byte on every
okf release and so break the drift check above for everyone who committed their
generated documentation. Name a more specific producer when you want one:
...
`--generated-by` takes any of the three OKF actor shapes —
`<producer>/<version>`, `human:<id>`, or `process:<id>` — and `okf trust` reports
the trust tier it implies.

`--okf-version MAJOR.MINOR` declares the OKF version in the generated bundle's
root `index.md`, exactly as `okf index --okf-version` does. Omitting it preserves
whatever declaration the destination already carries, so regenerating into a
directory that declares a version cannot silently strip it.

### Two things that will otherwise look like bugs

**A bundle generated with a date has no `log.md`, so do not check it with
`--log-enforce`.** Neither `--generated-at` nor the superseded v0.1 `--timestamp`
is written unless you ask — that is what keeps regeneration byte-identical — but
if you do supply one, every concept carries a date with no enclosing log:

```bash
okf profile document --profile docs/profiles/postgresql.dhall \
  --out /tmp/pg-profile --write --generated-at 2026-07-31T00:00:00Z
okf validate /tmp/pg-profile --strict
```
...
exit code `0`. They are advisories, so the command still succeeds — but
`okf validate --log-enforce` would fail. Generated documentation is not a
hand-maintained bundle.
...
okf validate examples/postgresql-profile \
  --profile docs/profiles/profile-documentation.dhall --profile-enforce --strict
```
...
The committed example is generated without `--generated-at` and without
`--timestamp` on purpose, so it has no varying input at all. It still satisfies
`okf validate --strict`, because `generated.by` alone is enough. Do not "fix"
anything by adding a date constant.

The meta-profile requires `generated` and declares both `generated.at` and the
superseded `timestamp` as *optional*. That is deliberate: `okf profile document`
provably never reads the clock, so declaring either one `recommended` would make
every bundle okf itself generates deviate from okf's own profile under
`--strict`. `docs/profiles/okf-v0-2.dhall` declares `generated.at` `recommended`
instead, and the two are meant to differ — that descriptor describes the format
in general, where a producer that knows the time should record it.
