...
- Decision: Always write the descriptor to `<bundle>/profile.dhall`. Provide no option to rename it.
  Rationale: Every consumer found in the Mori registry already pins its descriptor at `<bundle>/profile.dhall` (for example this repository's `docs/adr/profile.dhall`). The Seihou blueprints and the exec-plan skill's ADR workflow also assume that path. A fixed name keeps bootstrapped bundles interchangeable with blueprint-adopted ones. A rename option can come later if someone asks for it.
  Date: 2026-09-19
...
A **frozen import** is a Dhall import followed by `sha256:<64 hex digits>`, the semantic hash of what it evaluates to. Dhall refuses to use the import if the content ever changes, and it caches frozen imports by hash, so later evaluations work offline. The `dhall freeze` tool adds these hashes. Its Haskell entry points live in the `Dhall.Freeze` module of the `dhall` package, which this project already depends on (`dhall >=1.41 && <1.43`, currently 1.42.3). The relevant functions are `freezeRemoteImport :: FilePath -> Import -> IO Import`, which takes the directory against which relative imports resolve, and the `Scope` type (`OnlyRemoteImports | AllImports`). The source is on disk via `mori registry show dhall-lang/dhall-haskell --full`, under `dhall-haskell/dhall/src/Dhall/Freeze.hs`.
...
The header comment is fixed text plus the source and export, with no date and no okf version, so rendering is deterministic:
...
-- migration blueprints describe what each release requires.
let registry =
...
- A nested export round trip with `nested.decisions`.
- Hash preservation. Render `ImportRegistryExpression "https://example.invalid/package.dhall sha256:<64 zeros>" "a.b"` and assert that it succeeds without network, that the output contains the exact hash, and that the output ends with `registry.a.b`. This works because step 3 skips hashed imports.
- Refusal. `ImportRegistryExpression "./registry/package.dhall" "x"` returns `Left (RelativeExpressionImport _)`.
...
At the end of this milestone, `okf profile init [EXPORT] --bundle DIR [--registry REF]… [--no-local] [--date YYYY-MM-DD] [--write]` exists and works. Acceptance is the transcript in Concrete Steps: a greenfield directory validates as `OK: 0 concepts (okf_version 0.2)`, a second `--write` is refused, and `cabal test okf-cli` passes.
...
Export the record and its constructor from the module's export list next to `ProfileDocumentOptions (..)`, so tests can build it. In `profileCommandParser`, add `command "init"` with the description `"Bootstrap a profile into a bundle: pinned descriptor, version declaration, and log"`. Its options parser reuses `registryOption`, the same optional `EXPORT` `strArgument` as `profileDocumentOptionsParser`, and `noLocalSwitch`. Add a required `strOption (long "bundle" <> metavar "DIR" <> help "Bundle directory to bootstrap; created if missing")`, an optional `--date` whose metavar and validation match `okf log add`'s `--date` (reuse its parser if it is a standalone value), and a `--write` switch. Dispatch `ProfileInit options -> runProfileInit options` in `runProfile`.

`runProfileInit` runs in this order. Any failure exits 1 before anything is written.

1. Make `bundleDir` absolute and normalised. If `<bundle>/profile.dhall` exists, exit 1 with: "`<bundle>/profile.dhall` already exists; this bundle has already adopted a profile. To move its pin, follow the okf-profiles migration blueprint (`seihou agent migrate`) or edit the tag and re-run `dhall freeze`."
2. Select the profile. Call `resolveEffectiveProfileSources registryRefs noLocal`, then `loadProfileSourcesForNamedLookup`, then `selectEntry profiles export`. This reuses the exact lookup behavior of `okf profile show`, including its errors. Compile the selected spec with `compileProfileOrExit`, so that a profile okf cannot enforce is rejected before anything is written.
3. If `date` is given, validate it with the same check `okf log add` uses. Otherwise use `todayDate`.
...
5. Read the current declaration. If the bundle directory exists, call `readBundleVersion`; otherwise treat the declaration as nothing declared. Compute the target version with `bootstrapOkfVersion`.
6. If not `write`, print a preview and stop. The preview uses `renderIndexPreview` (the helper `profile document` uses) for `profile.dhall` with the rendered text. Then print one line each for the version declaration (`index.md: okf_version "0.2"`) and the log entry that would be appended (`log.md: ## 2026-09-19 / * **Adoption**: …`). End with `(preview only; pass --write to bootstrap <bundle>)`. Exit 0.
7. If `write`: create the directory with `createDirectoryIfMissing True` and write `<bundle>/profile.dhall`. Load it back with `loadProfileFile`. If loading fails, or the loaded spec differs from the selected spec, delete the file just written and exit 1 with the load error. This guarantees the pin on disk is the profile the user chose. Next, call `writeBundleIndexesWith targetVersion bundleDir`. Then append the log entry, reusing `runLogAdd bundleDir LogAddOptions {conceptId = Nothing, kind = "Adoption", message, date = Just entryDate}`. Take the record fields from its definition. The message is ``Adopt the `<profile name>` profile (`<export>`) from <renderProfileSourceLabel source>.``.
8. Print the summary, then validate. The summary names the written descriptor and the declared version. Then call `runValidate` with `bundlePath = Just bundleDir`, `profilePath = Just (bundleDir </> "profile.dhall")`, and every other flag off. Its output is the report, and its exit status becomes the command's. It exits 0 when the only findings are advisory profile deviations, and 1 only when the existing documents are structurally broken. The files have been written either way, and the summary printed before validation says so. Finish with two hint lines: `Enforce in CI: okf validate <bundle> --strict --profile <bundle>/profile.dhall --profile-enforce --log-enforce` and `Read the conventions: okf profile document --profile <bundle>/profile.dhall`.
...
If the suites print a different success line, record the actual line here and in Surprises & Discoveries.

The end-to-end greenfield transcript against the offline fixture registry, after Milestone 2:

```bash
rm -rf /tmp/okf-init-demo
cabal run -v0 okf -- profile init postgresql --no-local \
  --registry okf-core/test/fixtures/registry --bundle /tmp/okf-init-demo
```

Expected: the preview shows a `profile.dhall` whose import is a `../`-relative path into the repository's `okf-core/test/fixtures/registry/package.dhall` and whose body ends with `registry.postgresql`. It is followed by the `index.md` and `log.md` lines and `(preview only; pass --write to bootstrap /tmp/okf-init-demo)`, and `/tmp/okf-init-demo` still does not exist. Then:

```bash
cabal run -v0 okf -- profile init postgresql --no-local \
  --registry okf-core/test/fixtures/registry --bundle /tmp/okf-init-demo --write --date 2026-09-19
```

Expected output, approximately (exact wording is fixed during implementation and recorded here):

```text
Wrote /tmp/okf-init-demo/profile.dhall (postgresql from registry)
Declared okf_version "0.2" in /tmp/okf-init-demo/index.md
Wrote log.md for 2026-09-19
OK: 0 concepts (okf_version 0.2)
Enforce in CI: okf validate /tmp/okf-init-demo --strict --profile /tmp/okf-init-demo/profile.dhall --profile-enforce --log-enforce
Read the conventions: okf profile document --profile /tmp/okf-init-demo/profile.dhall
```

Running the same `--write` command again must fail:

```text
/tmp/okf-init-demo/profile.dhall already exists; this bundle has already adopted a profile. ...
```
...
```bash
rm -rf /tmp/okf-init-adr
cabal run -v0 okf -- profile init documentation.architectureDecisions --no-local \
  --bundle /tmp/okf-init-adr --write
cat /tmp/okf-init-adr/profile.dhall
```

Expected: the descriptor imports `https://raw.githubusercontent.com/shinzui/okf-profiles/v0.14.0/package.dhall` with the same `sha256:87d2…f50` hash as `defaultRegistryReference`, byte-for-byte, and selects `registry.documentation.architectureDecisions`. Validation prints `OK: 0 concepts (okf_version 0.2)`.
...
The greenfield transcript in Concrete Steps produces a bundle for which `okf validate /tmp/okf-init-demo --strict --profile /tmp/okf-init-demo/profile.dhall --profile-enforce --log-enforce` exits 0 and prints `OK: 0 concepts (okf_version 0.2)`. The refusal transcript exits 1 and leaves `profile.dhall` unchanged (compare with `shasum` before and after).

Adopting into an existing bundle leaves its concepts untouched:

```bash
rm -rf /tmp/okf-init-existing && cp -R okf-core/test/fixtures/valid-bundle /tmp/okf-init-existing
cabal run -v0 okf -- profile init postgresql --no-local \
  --registry okf-core/test/fixtures/registry --bundle /tmp/okf-init-existing --write
diff -r okf-core/test/fixtures/valid-bundle /tmp/okf-init-existing
```
...
Preview mode writes nothing and can be run any number of times. `--write` is deliberately not repeatable. A second run refuses because `profile.dhall` exists, and that refusal is the idempotence guarantee: it can never change a pin. If a `--write` run fails partway, the order of effects bounds the damage. A descriptor that fails to load back is deleted before anything else is touched. After a successful load, the index regeneration and the log append are the same operations `okf index --write` and `okf log add` perform, and both are safe to repeat. To retry a bootstrap from scratch, delete `<bundle>/profile.dhall` and the `Adoption` entry in `<bundle>/log.md` (or restore them with `git checkout -- <bundle>`), then run the command again. Tests use fresh temporary directories and remove them with `bracket … removeDirectoryRecursive`, following the existing `testProfileDocumentWritesBundle`.


## Interfaces and Dependencies

No new package dependencies are needed. `dhall` (already `>=1.41 && <1.43`) provides `Dhall.Parser.exprFromText`, `Dhall.Core` (`Expr (..)`, `Import (..)`, `ImportHashed (..)`, `ImportType (..)`, `FilePrefix (..)`, `Var (..)`, `makeBinding`, `makeFieldSelection`, `pretty`), and `Dhall.Freeze.freezeRemoteImport`. Confirm the exact export locations in the on-disk source found through `mori registry show dhall-lang/dhall-haskell --full` before writing imports; `Dhall.Core` re-exports the syntax types. `filepath` and `directory` are already dependencies of `okf-core`.
