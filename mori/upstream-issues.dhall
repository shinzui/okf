...
in  UpstreamIssues.UpstreamIssuesCatalog::{
    , entries =
      [ UpstreamIssues.UpstreamIssue::{
        ...
        , revisitTrigger = Some
            "When kivikakk/cmark-gfm-hs carries the footnote label on both node constructors (for example FOOTNOTE_DEFINITION Text), or accepts a patch doing so. No upstream ticket is filed yet. The C side already supports this: github/cmark-gfm commit c123e68 added CMARK_NODE_FOOTNOTE_DEFINITION to cmark_node_get_literal, and that commit is present in the vendored 0.29.0.gfm.13 sources (cbits/node.c lines 374-381 list both footnote node types), so only the Haskell binding's toNode conversion (CMarkGFM.hsc lines 366-369) needs changing. Two apparent workarounds are closed: the reverse conversion raises error \"constructing footnotes not supported\" for both types (CMarkGFM.hsc lines 520-521), so nodeToCommonmark on a footnote node crashes, and its renderer dereferences a parent_footnote_def pointer a reconstructed tree does not have. Note when patching that cbits/blocks.c lines 487-509 overwrite a MATCHED reference's literal with its ordinal number, so a matched reference yields \"1\" rather than a label; unmatched references and all definitions keep their label, and definitions plus unmatched references cover every label a document uses. okf needs this for OKF v0.2 per-claim attribution, where a footnote label is the join key into a concept's sources[].id (specification section 5.1). The same entry is mirrored in the corpus repo at mori://kivikakk/cmark-gfm-hs so other consumers discover it."
        , workaroundPath = Some
